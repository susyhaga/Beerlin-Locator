...
class Ranking(db.Model):
    ...
    def beer_rank(store):
        place = store
        sql = text('select beerbrand, round(avg(ranking),2) ranking from ranking where place = :value group by beerbrand order by ranking desc')
        result = db.engine.execute(sql, value = place)
        return_beerbrand = []
        return_ranking = []
        for r in result:
