...
from beerlin import app
from urllib import parse
import requests


GOOGLE_MAPS_API_KEY = 'AIzaSyBMd_Sabn9rfbMx7e9157nDFToMcTQAwvo'
# I will collect the updated users in an array, so I can store that in a new file at the end
updated_users = []

with open("beerlin/resources/locals_fake.txt", "r") as a_file:
  for line in a_file:
    id,name,address,latitude,longitude,zipcode = line.strip().split(",")
    ...
    full_address = address.replace('"', '')+", Berlin"
...
with open("beerlin/resources/dummy-users-v2.txt", "w") as txt_file:
